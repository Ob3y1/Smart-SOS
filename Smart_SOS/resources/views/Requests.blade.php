  ...
  <main class="d-flex flex-nowrap">
    <div id="sidebarId" class="sidebar position-relative d-flex flex-column flex-shrink-0 p-3 bg-body-tertiary ">
        ...
        <ul class="nav nav-pills flex-column mb-auto">
          ...
          <li class="nav-item">
            <a href="{{ route('requests') }}" class=" active nav-link link-body-emphasis">
              <svg class="bi pe-none me-2" width="16" height="16">
    ...
    <nav class="navbar bg-body-tertiary">
        <div class="container-fluid justify-content-md-between">
            <a class="navbar-brand fw-normal fs-4 mb-1 mb-md-0 d-sm-block">Emergency Requests</a>
        </div>
    </nav>
        ...
        <tbody id="userTable">
            @foreach ($groups as $group)
                  <tr>
                    ...
                    <td>{{ $group->EmergencyRequest->User->name ?? 'N/A' }}</td>
                    <td>{{ $group->EmergencyRequest->User->Otp->identifier ?? 'N/A' }}</td>
                    <td>{{ $group->Group->Job->title ?? 'N/A' }}</td>
                    <td>{{ $group->Group->car_number ?? 'N/A' }}</td>
                    <td>{{ $group->Group->Site->name ?? 'N/A' }}</td>
                    <td>{{ $group->R_status->title ?? 'N/A' }}</td>
                    <td id="location-{{$group->id}}">جاري التحميل...</td>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        @foreach ($groups as $group)
            convertToAddress({{ $group->EmergencyRequest->latitude }}, {{ $group->EmergencyRequest->longitude }}, 'location-{{$group->id}}');
        @endforeach
    });
</script>
</html>